...
import { updateFile, updateFileSchema } from "../schema/inputSchema";
...
const FileEdit = ({ file, onClose ,fileId}: FileEditProps) => {
  const {
    register,
    setValue,
    handleSubmit,
    ...
    defaultValues: {
      ...
      department_ids: file.department_ids,
      duedate: file.duedate
        ? new Date(file.duedate).toISOString().slice(0, 16)
        : "",
      sender: file.sender,
      receiver: file.receiver,
      visible: file.visible,
      important: file.important,
      inboxoroutbox: file.inboxoroutbox,
    },
  });
  ...
  const onSubmit: SubmitHandler<updateFile> = async (data) => {
    console.log(file.id)
    console.log(data)
    const {data: UpdateData,error: UpdateError} = await supabase.from("files").update({
      name: data.name,
